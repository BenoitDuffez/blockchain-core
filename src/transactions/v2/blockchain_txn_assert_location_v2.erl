...
calculate_staking_fee(Txn, Chain) ->
    ...
    Fee = blockchain_ledger_v1:staking_fee_txn_assert_location_v2(Ledger),
...
print(#blockchain_txn_assert_location_v2_pb{
    ...
    io_lib:format("type=assert_location, gateway=~p, owner=~p, payer=~p, location=~p, owner_signature=~p, payer_signature=~p, nonce=~p, staking_fee=~p, fee=~p",
                  [?TO_ANIMAL_NAME(Gateway), ?TO_B58(Owner), ?TO_B58(Payer),
		   Loc, OS, PS, Nonce, StakingFee, Fee]).
